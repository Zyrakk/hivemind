...
// Each component of a compound command (split on &&) must start with one of these prefixes.
var allowedChecklistPrefixes = []string{
	"go build", "go test", "go vet", "go run",
	"test -f", "test -d", "test -e",
	"grep ", "grep -",
	"cat ", "head ", "tail ", "wc ",
	"ls ", "ls", "find ", "tree ", "tree",
	"stat ",
	"cd ",
}

// deniedChecklistPrefixes rejects dangerous commands even if they match an allowed prefix.
var deniedChecklistPrefixes = []string{
...
func isAllowedChecklistCommand(cmd string) bool {
	...
	// Split compound commands on && and validate each component.
	...
	for _, part := range parts {
		...
		for _, denied := range deniedChecklistPrefixes {
			if strings.HasPrefix(part, denied) {
				return false
			}
		}
		allowed := false
...
func (e *Evaluator) EvaluateWorkerOutput(ctx context.Context, session launcher.Session) (*EvalResult, error) {
	...
	// Determine evaluation detail for progress notification
	evalDetail := "engine evaluation"
